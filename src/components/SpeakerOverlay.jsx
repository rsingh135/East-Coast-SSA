...
export function SpeakerOverlay({ speaker, open, onClose }) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-90 overflow-y-auto bg-paper text-near-black"
    >
      <div className="mx-auto grid min-h-full max-w-6xl grid-cols-1 gap-8 px-4 py-[calc(var(--frame-bar)+2rem)] md:grid-cols-2 md:gap-14 md:px-8">
        ...
        <div className="flex flex-col justify-center gap-6">
          <p className="label text-accent-deep">{speaker.badge}</p>

          <div>
            <h2 className="display text-display">{speaker.name}</h2>
            <p className="label mt-4 text-near-black/60">
              {speaker.role} · {speaker.org}
            </p>
          </div>

          <p className="max-w-prose text-lg leading-relaxed font-light text-near-black/80">{speaker.bio}</p>

          <button
            ref={closeRef}
            type="button"
            onClick={onClose}
            className="label group flex w-fit items-center gap-3 rounded-full border border-near-black/25 py-3 pr-4 pl-5 transition-colors duration-300 hover:border-accent-deep hover:text-accent-deep"
          >
            Close
